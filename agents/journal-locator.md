...
```
<journal>/
├── HOUSE_RULES.md      # standing rules (may list extra notes directories to search)
├── research/           # research docs, YYYY-MM-DD[-<work-item>]-<desc>.md
├── plans/              # implementation plans (status handshake in frontmatter) and supervisor kickoff prompts
...
2. **Grep for content, glob for filenames.** Research and plan filenames are dated and often
   carry the work item ID — glob for `*<ITEM>*` there; notes elsewhere may be topic-named,
   so grep file CONTENT for IDs as well as globbing filenames.
