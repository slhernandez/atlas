...
repo's CLAUDE.md for its layout and conventions; pass both paths to every agent you spawn.

## Steps

1. **Gather the inputs first, in the main context.** If the argument is a work item, fetch it
   per the config's tracker — description AND comments; on sparse tickets the requirements
   hide in the comments. With tracker `manual`, ask for the brief. If the user names files
   (tickets, docs, JSON), read them entirely so decomposition starts from full context.
...
3. **Spawn parallel read-only subagents**, each told WHAT to find, never HOW to search:
   ...
   - the built-in `Explore` agent — broad sweeps that fit none of the above
4. **Synthesize.** Live findings outrank journal findings. Connect components; keep every
   claim pinned to file:line. **Verify before writing:** re-read the two or three load-bearing
   claims at their cited locations yourself — the document is persistent and will be trusted.
...
5. **Write the research document** from the journal's research template to
   ...
   none). Frontmatter carries the real date/time, git commit, and branch — never
   placeholders. Include the Scope Assessment when researching a work item: what will change,
...
6. **Present findings:** the direct answer first, then the key file references, then the
   ...
   many call sites), hand the research doc to `/atlas:feature-workflow`, whose Phase 0 starts
